...
class TestOpenAITranscription:
    ...
    async def test_transcribe_audio_custom_params(self, openai_service, sample_audio_file):
        ...
        # Act
        await openai_service.transcribe_audio(sample_audio_file, language="en", prompt="Captain's log entry")
    ...
    async def test_transcribe_large_audio_file(self, openai_service, large_audio_file, tmp_path):
        ...
        assert total_time < 0.025, f"Parallel execution should be faster than sequential. Took {total_time:.3f}s"
...
class TestOpenAISummary:
    ...
    async def test_generate_summary_api_error(self, openai_service):
        ...
        with pytest.raises(SummaryError, match="Summary generation failed"):
            await openai_service.generate_summary(
                "This is a much longer test transcription that contains enough words to trigger the actual API call and test the error handling functionality properly for the summary generation process."
            )
    ...
    async def test_generate_summary_empty_response(self, openai_service):
        ...
        with pytest.raises(SummaryError, match="Empty summary"):
            await openai_service.generate_summary(
                "This is a much longer test transcription that contains enough words to trigger the actual API call and test the empty response handling functionality properly for the summary generation process."
            )
...
class TestOpenAIServiceIntegration:
    ...
    async def test_embedding_similarity(self, openai_service):
        ...
        openai_service.async_client.embeddings.create.side_effect = lambda input, **kwargs: mock_embedding_response(
            input, **kwargs
        )

...
from app.services.network_resilient_processor import NetworkResilientProcessor, NetworkTask, TaskType, TaskPriority
...
async def resilient_processor(mock_settings, mock_db_session):
    ...
    processor = NetworkResilientProcessor(settings=mock_settings, db_session=mock_db_session)
    return processor
...
class TestNetworkResilientProcessor:
    ...
    async def test_queue_s3_upload(self, resilient_processor):
        ...
        task_id = await resilient_processor.queue_s3_upload(log_entry_id, test_file, is_video=False)
    ...
    async def test_queue_transcription(self, resilient_processor):
        ...
        task_id = await resilient_processor.queue_transcription(log_entry_id, audio_file=test_file)

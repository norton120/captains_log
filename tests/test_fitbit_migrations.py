...
class TestFitbitMigrationTables:
    ...
    async def test_migration_creates_user_fitbit_settings_table(self, async_db_session: AsyncSession):
        """Verify user_fitbit_settings table exists with correct columns."""
        inspector = inspect(async_db_session.bind)

        # Check table exists
        tables = await async_db_session.run_sync(lambda sync_session: inspector.get_table_names())
        assert "user_fitbit_settings" in tables

        # Check columns
        columns = await async_db_session.run_sync(lambda sync_session: inspector.get_columns("user_fitbit_settings"))
    ...
    async def test_migration_creates_fitbit_data_table(self, async_db_session: AsyncSession):
        ...
        columns = await async_db_session.run_sync(lambda sync_session: inspector.get_columns("fitbit_data"))
        column_names = [col["name"] for col in columns]
    ...
    async def test_migration_adds_user_preferences_fitbit_columns(self, async_db_session: AsyncSession):
        """Verify UserPreferences table has Fitbit OAuth columns."""
        inspector = inspect(async_db_session.bind)

        columns = await async_db_session.run_sync(lambda sync_session: inspector.get_columns("user_preferences"))
        column_names = [col["name"] for col in columns]
...
class TestFitbitMigrationIndexes:
    ...
    async def test_migration_creates_user_fitbit_settings_indexes(self, async_db_session: AsyncSession):
        """Verify indexes on user_fitbit_settings table."""
        inspector = inspect(async_db_session.bind)

        indexes = await async_db_session.run_sync(lambda sync_session: inspector.get_indexes("user_fitbit_settings"))
        ...
        user_id_unique = [uc for uc in unique_constraints if "user_id" in uc["column_names"]]
        assert len(user_id_unique) > 0, "Missing unique constraint on user_id"

    async def test_migration_creates_fitbit_data_indexes(self, async_db_session: AsyncSession):
        """Verify indexes on fitbit_data table."""
        inspector = inspect(async_db_session.bind)

        indexes = await async_db_session.run_sync(lambda sync_session: inspector.get_indexes("fitbit_data"))

        # Should have index on log_entry_id
        log_entry_indexes = [idx for idx in indexes if "log_entry_id" in idx["column_names"]]
        ...
        log_entry_unique = [uc for uc in unique_constraints if "log_entry_id" in uc["column_names"]]
...
class TestFitbitMigrationForeignKeys:
    ...
    async def test_user_fitbit_settings_foreign_keys(self, async_db_session: AsyncSession):
    ...
    async def test_fitbit_data_foreign_keys(self, async_db_session: AsyncSession):
        ...
        foreign_keys = await async_db_session.run_sync(lambda sync_session: inspector.get_foreign_keys("fitbit_data"))

        # Should have foreign key to log_entries table
        log_entries_fk = [fk for fk in foreign_keys if fk["referred_table"] == "log_entries"]
...
class TestFitbitMigrationColumnTypes:
    ...
    async def test_user_fitbit_settings_column_types(self, async_db_session: AsyncSession):
...
class TestFitbitMigrationRollback:
    ...
    async def test_downgrade_preserves_other_tables(self, async_db_session: AsyncSession):

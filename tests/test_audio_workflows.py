...
class TestTranscribeAudioStep:
    ...
    async def test_transcribe_audio_step_success(self, audio_workflow, sample_audio_file):
        ...
        audio_workflow.openai_service.transcribe_audio.assert_called_once_with(
            sample_audio_file, prompt="Captain's log entry from sailing vessel"
        )
...
class TestGenerateSummaryStep:
    ...
    async def test_generate_summary_step_success(self, audio_workflow):
        ...
        audio_workflow.openai_service.generate_summary.assert_called_once_with(
            transcription, instructions="Focus on key events, weather conditions, and important decisions."
        )
...
class TestUpdateLogEntryStep:
    ...
    async def test_update_log_entry_step_error(self, audio_workflow, log_entry_factory):
        ...
        result = await step.execute(
            log_entry_id=log_entry.id, processing_status=ProcessingStatus.FAILED, processing_error="Test error message"
        )
    ...
    async def test_update_log_entry_step_not_found(self, audio_workflow):
        ...
        with pytest.raises(WorkflowError, match="Log entry not found"):
            await step.execute(log_entry_id=fake_id, transcription="Test")
...
class TestAudioProcessingWorkflow:
    ...
    @pytest.mark.workflow
    async def test_complete_audio_processing_workflow(self, audio_workflow, sample_audio_file, log_entry_factory):
        ...
        # Act
        result = await audio_workflow.process_audio(log_entry_id=log_entry.id, audio_file=sample_audio_file)
    ...
    @pytest.mark.integration
    @pytest.mark.workflow
    async def test_workflow_error_handling(self, audio_workflow, sample_audio_file, log_entry_factory):
        ...
        with pytest.raises(WorkflowError):
            await audio_workflow.process_audio(log_entry_id=log_entry.id, audio_file=sample_audio_file)
    ...
    @pytest.mark.workflow
    @pytest.mark.db
    async def test_workflow_status_updates(self, audio_workflow, sample_audio_file, log_entry_factory):
    ...
    @pytest.mark.integration
    @pytest.mark.workflow
    async def test_workflow_retry_logic(self, audio_workflow, sample_audio_file, log_entry_factory):
    ...
    @pytest.mark.integration
    @pytest.mark.workflow
    async def test_concurrent_workflows(self, test_settings, async_db_engine, sample_audio_file, log_entry_factory):
        ...
        for i in range(3):
            ...
            workflow = AudioProcessingWorkflow(
                settings=test_settings, db_session=session, s3_service=s3_mock, openai_service=openai_mock
            )
        ...
        try:
            ...
            tasks = [workflows[i].process_audio(log_entries[i].id, sample_audio_file) for i in range(3)]

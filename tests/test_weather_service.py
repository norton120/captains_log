...
def mock_coops_temp_response():
    ...
    return {"data": [{"t": "2025-01-01 12:00", "v": "72.5"}]}  # temperature Fahrenheit


@pytest.mark.asyncio
async def test_get_marine_conditions_success(
    weather_service, mock_weather_forecast_response, mock_coops_wind_response, mock_coops_temp_response
):
...
def test_parse_coops_response(weather_service):
    ...
    wind_data = {"data": [{"t": "2025-01-01 12:00", "s": "10.5", "d": "270", "g": "15.2"}]}
    ...
    temp_data = {"data": [{"t": "2025-01-01 12:00", "v": "72.5"}]}
...
def test_combine_weather_data(weather_service):
    ...
    forecast_data = {
        "properties": {"windSpeed": {"values": [{"value": 5.0}]}, "temperature": {"values": [{"value": 20.0}]}}
    }

    station_data = {"wind_speed_kts": 12.0, "air_temp_f": 75.0}  # Should override forecast

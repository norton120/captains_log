...
class TestLogUploadEndpoint:
    ...
    @pytest.mark.api
    @pytest.mark.unit
    async def test_upload_valid_audio_success(self, api_client, upload_file_factory, mock_workflow_service):
        """Should successfully upload valid audio file and start processing."""
        # Arrange
        upload_file = upload_file_factory("valid_short")

        with (
            patch("app.api.logs.AudioProcessingWorkflow") as mock_workflow_class,
            ...
            response = await api_client.post(
                "/api/logs/upload", files={"file": (upload_file.filename, upload_file.file, upload_file.content_type)}
            )
...
class TestLogListEndpoint:
    ...
    async def test_list_logs_status_filter(self, api_client, log_entry_factory):
        ...
        completed_entries = [await log_entry_factory(processing_status=ProcessingStatus.COMPLETED) for _ in range(3)]
        pending_entries = [await log_entry_factory(processing_status=ProcessingStatus.PENDING) for _ in range(2)]
    ...
    async def test_list_logs_pagination(self, api_client, log_entry_factory):
        ...
        entries = [await log_entry_factory(audio_s3_key=f"test/audio_{i}.wav") for i in range(15)]
...
class TestLogStatusEndpoint:
    ...
    async def test_get_log_status_success(self, api_client, log_entry_factory):
        ...
        # Arrange
        log_entry = await log_entry_factory(processing_status=ProcessingStatus.VECTORIZING)
    ...
    async def test_get_log_status_failed(self, api_client, log_entry_factory):
        ...
        # Arrange
        log_entry = await log_entry_factory(
            processing_status=ProcessingStatus.FAILED, processing_error="OpenAI API rate limit exceeded"
        )
...
class TestLogAudioEndpoint:
    ...
    async def test_get_log_audio_success(self, api_client, log_entry_factory, mock_s3_service):
        ...
        # Arrange
        log_entry = await log_entry_factory(audio_s3_key="test/audio_file.wav")
    ...
    async def test_get_log_audio_file_missing(self, api_client, log_entry_factory):
        ...
        # Arrange
        log_entry = await log_entry_factory(audio_s3_key="test/missing_audio.wav")
...
class TestAPIErrorHandling:
    ...
    async def test_malformed_request_data(self, api_client):
        ...
        response = await api_client.post(
            "/api/logs/upload", data="invalid-json-data", headers={"content-type": "application/json"}
        )
...
class TestAPIIntegrationScenarios:
    ...
    async def test_upload_then_status_check(self, api_client, upload_file_factory):
        ...
        with (
            patch("app.api.logs.AudioProcessingWorkflow") as mock_workflow,
            patch("app.services.s3.S3Service.upload_audio") as mock_s3_upload,
        ):
    ...
    async def test_audio_access_after_processing(self, api_client, log_entry_factory):
        ...
        # Arrange
        log_entry = await log_entry_factory(
            processing_status=ProcessingStatus.COMPLETED, audio_s3_key="test/completed_audio.wav"
        )
...
class TestLogRetryEndpoint:
    ...
    async def test_retry_pending_log_error(self, api_client, log_entry_factory):
        ...
        # Arrange
        log_entry = await log_entry_factory(
            processing_status=ProcessingStatus.PENDING, audio_s3_key="test/pending_audio.wav"
        )
    ...
    async def test_retry_log_no_audio_error(self, api_client, log_entry_factory):
        ...
        # Arrange
        log_entry = await log_entry_factory(
            processing_status=ProcessingStatus.FAILED, audio_s3_key=None, audio_local_path=None
        )

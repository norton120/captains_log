...
class Settings(BaseSettings):
    ...
    allow_new_user_registration: bool = Field(
        default=True, description="Allow new user registration (always allowed when no users exist)"
    )
    ...
    media_storage_mode: MediaStorageMode = Field(
        default=MediaStorageMode.S3_ONLY, description="Media storage mode: s3_only or local_with_s3"
    )
    local_media_path: Optional[str] = Field(
        default="./media", description="Local path for media storage when using local_with_s3 mode"
    )

    # Audio processing settings
    max_audio_file_size: int = Field(default=100 * 1024 * 1024, description="Max audio file size in bytes (100MB)")
    allowed_audio_formats: list[str] = Field(
        default=["mp3", "mp4", "mpeg", "mpga", "m4a", "wav", "webm"], description="Allowed audio file formats"
    )
    ...
    allowed_video_formats: list[str] = Field(
        default=["mp4", "webm", "mov", "avi"], description="Allowed video file formats"
    )

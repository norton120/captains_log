...
class S3Service:
    ...
    def _generate_s3_key(self, file_path: Path) -> str:
        ...
        return f"{self.settings.s3_audio_prefix}{timestamp}/{unique_id}{file_extension}"
    ...
    def _validate_audio_file(self, file_path: Path) -> None:
        ...
        if file_size > self.settings.max_audio_file_size:
            size_mb = file_size / (1024 * 1024)
            max_mb = self.settings.max_audio_file_size / (1024 * 1024)
        ...
        if file_extension not in self.settings.allowed_audio_formats:
            raise AudioUploadError(
                ...
                f"Allowed formats: {', '.join(self.settings.allowed_audio_formats)}"
            )

...
class S3Service:
    ...
    def __init__(self, settings: Settings):
        ...
        self.config = Config(region_name=settings.aws_region, retries={"max_attempts": 3, "mode": "adaptive"})
    ...
    def _validate_media_file(self, file_path: Path, is_video: bool = False) -> None:
        ...
        if file_size > max_size:
            ...
            raise AudioUploadError(f"File size exceeds limit: {size_mb:.1f}MB > {max_mb}MB")

        # Check file format
        file_extension = file_path.suffix.lower().lstrip(".")
        if file_extension not in allowed_formats:
            raise AudioUploadError(
                f"Unsupported {media_type} format: {file_extension}. " f"Allowed formats: {', '.join(allowed_formats)}"
            )
    ...
    async def upload_audio(self, file_path: Path) -> str:
        ...
        try:
            ...
            await loop.run_in_executor(None, self._upload_file_sync, file_path, s3_key)
    ...
    async def upload_video(self, file_path: Path) -> str:
        ...
        try:
            ...
            # Upload file to S3 in thread pool to avoid blocking
            loop = asyncio.get_event_loop()
            await loop.run_in_executor(None, self._upload_video_file_sync, file_path, s3_key)
    ...
    def _upload_video_file_sync(self, file_path: Path, s3_key: str) -> None:
        ...
        try:
            self.s3_client.upload_file(
                ...
                s3_key,
                ExtraArgs={"ContentType": self._get_content_type(file_path, True), "ServerSideEncryption": "AES256"},
            )
    ...
    def _upload_file_sync(self, file_path: Path, s3_key: str) -> None:
        ...
        try:
            self.s3_client.upload_file(
                ...
                self.settings.s3_bucket_name,
                s3_key,
                ExtraArgs={"ContentType": self._get_content_type(file_path, False), "ServerSideEncryption": "AES256"},
            )
        except ClientError as e:
            error_code = e.response["Error"]["Code"]
            if error_code == "NoSuchBucket":
    ...
    async def get_audio_url(self, s3_key: str, expiry_seconds: Optional[int] = None) -> str:
        """
        ...
        try:
            ...
            loop = asyncio.get_event_loop()
            exists = await loop.run_in_executor(None, self._check_object_exists, s3_key)

            if not exists:
                raise AudioNotFoundError(f"Audio file not found: {s3_key}")

            # Generate presigned URL
            url = await loop.run_in_executor(None, self._generate_presigned_url, s3_key, expiry_seconds)
    ...
    async def get_video_url(self, s3_key: str, expiry_seconds: Optional[int] = None) -> str:
        """
    ...
    def _check_object_exists(self, s3_key: str) -> bool:
        ...
        try:
            self.s3_client.head_object(Bucket=self.settings.s3_bucket_name, Key=s3_key)
            return True
        except ClientError as e:
    ...
    def _generate_presigned_url(self, s3_key: str, expiry_seconds: int) -> str:
        ...
        try:
            return self.s3_client.generate_presigned_url(
                "get_object", Params={"Bucket": self.settings.s3_bucket_name, "Key": s3_key}, ExpiresIn=expiry_seconds
            )
        except ClientError as e:
    ...
    async def delete_audio(self, s3_key: str) -> bool:
        ...
        try:
            loop = asyncio.get_event_loop()
            await loop.run_in_executor(None, self._delete_object_sync, s3_key)
            return True
    ...
    def _delete_object_sync(self, s3_key: str) -> None:
        ...
        self.s3_client.delete_object(Bucket=self.settings.s3_bucket_name, Key=s3_key)
    ...
    async def get_file_metadata(self, s3_key: str) -> dict:
        ...
        try:
            loop = asyncio.get_event_loop()
            metadata = await loop.run_in_executor(None, self._get_object_metadata_sync, s3_key)
    ...
    def _get_object_metadata_sync(self, s3_key: str) -> dict:
        ...
        response = self.s3_client.head_object(Bucket=self.settings.s3_bucket_name, Key=s3_key)
        return response
    ...
    async def check_connectivity(self) -> bool:
        ...
        try:
            loop = asyncio.get_event_loop()
            await loop.run_in_executor(None, self._check_bucket_access_sync)
            return True
        except Exception:
            return False

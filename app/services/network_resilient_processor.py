...
class NetworkResilientProcessor:
    ...
    async def queue_s3_upload(
        self, log_entry_id: str, file_path: Path, is_video: bool = False, priority: TaskPriority = TaskPriority.HIGH
    ) -> str:
        ...
        task = NetworkTask(
            ...
            payload={"file_path": str(file_path), "is_video": is_video},
        )
    ...
    async def queue_transcription(
        ...
        task = NetworkTask(
            ...
            priority=priority,
            log_entry_id=log_entry_id,
            payload={"audio_file": str(audio_file) if audio_file else None, "s3_key": s3_key, "local_path": local_path},
        )
    ...
    async def _process_pending_tasks(self) -> None:
        ...
        tasks = [self._process_task_with_semaphore(task, semaphore) for task in pending_tasks[:10]]  # Limit batch size
    ...
    async def _execute_summary(self, task: NetworkTask) -> None:
        ...
        await self._update_log_entry(
            task.log_entry_id, {"summary": summary, "processing_status": ProcessingStatus.COMPLETED}
        )
    ...
    async def _mark_log_entry_failed(self, log_entry_id: str, error_message: str) -> None:
        ...
        try:
            await self._update_log_entry(
                log_entry_id, {"processing_status": ProcessingStatus.FAILED, "processing_error": error_message}
            )
        except Exception as e:
    ...
    async def get_queue_status(self) -> Dict[str, Any]:
        ...
        async with self._queue_lock:
            ...
            failed_tasks = sum(
                1 for task in self.task_queue.values() if not task.is_completed and task.retry_count >= task.max_retries
            )
...
@DBOS.workflow()
async def resilient_media_processing_workflow(log_entry_id: str, media_file_path: str, settings_dict: dict) -> dict:
    """
    ...
    try:
        ...
        try:
            ...
            upload_task_id = await processor.queue_s3_upload(log_entry_id, media_path, is_video=is_video)
        ...
        return {"workflow_id": log_entry_id, "status": "failed", "error": str(e)}

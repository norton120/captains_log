...
class AudioChunker:
    ...
    async def chunk_audio_file(
        self, audio_file: Path, chunk_duration_seconds: int = 600  # 10 minutes default
    ) -> List[Path]:
        """
        ...
        try:
            ...
            chunk_duration = await self._calculate_optimal_chunk_duration(file_size, duration, chunk_duration_seconds)

            logger.info(f"Splitting {duration:.1f}s audio into {chunk_duration}s chunks")

            # Split audio file into chunks
            chunk_files = await self._split_audio_into_chunks(audio_file, chunk_duration, duration)
    ...
    async def _calculate_optimal_chunk_duration(self, file_size: int, duration: float, max_chunk_duration: int) -> int:
        """
    ...
    async def _split_audio_into_chunks(
        self, audio_file: Path, chunk_duration: int, total_duration: float
    ) -> List[Path]:
        """
        ...
        try:
            ...
            for i in range(num_chunks):
                ...
                with tempfile.NamedTemporaryFile(
                    delete=False, suffix=audio_file.suffix, prefix=f"chunk_{i:03d}_"
                ) as temp_file:
                    chunk_path = Path(temp_file.name)

                # Extract chunk using ffmpeg
                await self._extract_audio_chunk(audio_file, chunk_path, start_time, chunk_duration)

                # Validate chunk was created
                if chunk_path.exists() and chunk_path.stat().st_size > 0:
                    chunk_files.append(chunk_path)
                    logger.debug(
                        f"Created chunk {i}: {chunk_path.name} " f"({chunk_path.stat().st_size / (1024*1024):.1f}MB)"
                    )
                else:
    ...
    async def _extract_audio_chunk(self, input_file: Path, output_file: Path, start_time: int, duration: int) -> None:
        """
        ...
        try:
            ...
            output_stream = ffmpeg.output(
                input_stream, str(output_file), acodec="copy", loglevel="error"  # Copy audio codec to avoid re-encoding
            )
            ...
            if process.returncode != 0:
                ...
                raise AudioChunkingError(f"FFmpeg chunk extraction failed with code {process.returncode}: {error_msg}")

            logger.debug(f"FFmpeg chunk extraction completed successfully")

        except FileNotFoundError:
            raise AudioChunkingError("FFmpeg not found. Please install FFmpeg to chunk audio files.")
        except Exception as e:

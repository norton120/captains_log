        ...
        try:
            ...
            if self.video_processor.is_video_file(media_file):
                ...
                audio_file = await self.video_processor.extract_audio_from_video(
                    media_file, output_format="wav", sample_rate=44100
                )
...
class StoreVideoStep(BaseWorkflowStep):
    ...
    async def execute(self, video_file: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"success": True, "file_size": video_file.stat().st_size, **storage_result}
...
class StoreAudioStep(BaseWorkflowStep):
    ...
    async def execute(self, audio_file: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"success": True, "file_size": audio_file.stat().st_size, **storage_result}
...
class TranscribeAudioStep(BaseWorkflowStep):
    ...
    async def execute(
        self, audio_file: Optional[Path] = None, s3_key: Optional[str] = None, local_path: Optional[str] = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            transcription = await self.openai_service.transcribe_audio(
                audio_file, prompt="Captain's log entry from sailing vessel"
            )

            logger.info(f"Successfully transcribed audio: {len(transcription)} characters")
            return {"success": True, "transcription": transcription, "character_count": len(transcription)}
...
class GenerateEmbeddingStep(BaseWorkflowStep):
    ...
    async def execute(self, transcription: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"success": True, "embedding": embedding, "embedding_dimensions": len(embedding)}
    ...
    async def execute(self, transcription: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if not transcription or not transcription.strip():
                ...
                return {"success": True, "log_type": "SHIP", "classification_source": "default"}
            ...
            return {"success": True, "log_type": log_type, "classification_source": "llm"}

        except Exception as e:
            logger.error(f"Log classification failed: {e}, defaulting to SHIP")
            # Default to SHIP on failure
            return {"success": True, "log_type": "SHIP", "classification_source": "fallback", "error": str(e)}
...
class GenerateSummaryStep(BaseWorkflowStep):
    ...
    async def execute(self, transcription: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"success": True, "summary": summary, "summary_length": len(summary)}
...
class UpdateLogEntryStep(BaseWorkflowStep):
    ...
    async def execute(self, log_entry_id: UUID, **update_data) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"success": True, "log_entry_id": str(log_entry_id), "updated_fields": list(update_data.keys())}
...
class AudioProcessingWorkflow:
    ...
    def __init__(
        ...
        if self.use_resilient_processor:
            self.resilient_processor = NetworkResilientProcessor(settings, db_session, None, self.openai_service)
        else:
    ...
    async def process_media(self, log_entry_id: UUID, media_file: Path, max_retries: int = 3) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            preprocess_result = await self._retry_step(
                "preprocess", self.preprocess_step.execute, media_file, max_retries=max_retries
            )
            ...
            if is_video and video_file:
                video_store_result = await self._retry_step(
                    "store_video", self.store_video_step.execute, video_file, max_retries=max_retries
                )
            ...
            audio_store_result = await self._retry_step(
                "store_audio", self.store_audio_step.execute, audio_file, max_retries=max_retries
            )
            ...
            classify_result = await self._retry_step(
                "classify", self.classify_step.execute, transcribe_result["transcription"], max_retries=max_retries
            )
            ...
            embedding_result = await self._retry_step(
                "embedding", self.embedding_step.execute, transcribe_result["transcription"], max_retries=max_retries
            )
            ...
            summary_result = await self._retry_step(
                "summary", self.summary_step.execute, transcribe_result["transcription"], max_retries=max_retries
            )
            ...
            try:
                await self.update_step.execute(
                    log_entry_id, processing_status=ProcessingStatus.FAILED, processing_error=error_msg
                )
    ...
    async def process_media_resilient(self, log_entry_id: UUID, media_file: Path) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            try:
                await self.update_step.execute(
                    log_entry_id, processing_status=ProcessingStatus.FAILED, processing_error=str(e)
                )
            except Exception as update_error:
                logger.error(f"Failed to update error status: {update_error}")
    ...
    async def _retry_step(self, step_name: str, step_func, *args, max_retries: int = 3, **kwargs) -> Dict[str, Any]:
        ...
        # All retries failed
        raise WorkflowError(f"{step_name} step failed after {max_retries} attempts: {last_exception}")
...
@DBOS.workflow()
async def process_audio_workflow(log_entry_id: str, audio_file_path: str, settings_dict: dict) -> dict:
    """
